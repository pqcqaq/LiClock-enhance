...
void AppManager::showAppList(int page)
{
    ...
    // 下面是标题部分
    {
        ...
        if (WiFi.isConnected())
        {
            display.drawXBitmap(x - 16, 2, wifiIcon_bits, 16, 13, 0);
            x -= 16 - 2;
        }
    }
    // 返回按钮
    display.drawXBitmap(12, 21, goBackIcon_bits, 32, 32, 0);
    ...
    for (int16_t i = 0; i < pageItemsCount; i++)
    {
        ...
        {
            display.drawXBitmap(x + 8, y, defaultAppIcon_bits, 32, 32, 0);
        }
