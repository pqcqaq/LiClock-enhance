...
void task_hal_update(void *) {
    while (1) {
        if (hal._hookButton) {
            while (hal.btnr.isPressing() || hal.btnl.isPressing() ||
                   hal.btnc.isPressing()) {
                hal.btnr.tick();
                hal.btnl.tick();
                hal.btnc.tick();
            ...
            while (hal._hookButton) {
                while (hal.SleepUpdateMutex) delay(10);
                hal.update();
                delay(20);
            }
            while (hal.btnr.isPressing() || hal.btnl.isPressing() ||
...
void task_btn_buzzer(void *) {
    ...
    while (1) {
        ...
        if (!hal.btnl.isPressing() && buz_l)
            ...
            buz_r = false;
        if (!hal.btnc.isPressing() && buz_c) buz_c = false;
        delay(50);
    }
}
void cfu_task(void *) {
    ...
    if (file_true) {
        if (strcmp(cfu["version"].as<String>().c_str(), code_version) != 0) {
            ESP_LOGW(
                "HAL",
                "当前版本为 %s，最新版本为 %s，建议尽快升级以获得最佳体验。",
                code_version, cfu["version"].as<String>().c_str());
        ...
        if (hal.has_new_firmware) {
            strcpy(hal.update_url[0],
                   cfu["updateinfo"]["url"].as<String>().c_str());
            strcpy(hal.update_url[1],
                   cfu["updateinfo"]["url1"].as<String>().c_str());
...
char *HAL::get_char_sha_key(const char *str) {
    ...
    ets_sha_update(&ctx, SHA2_256, (const uint8_t *)str,
                   strlen(str) * 8);       // 更新哈希值
...
#define url_firmware \
    "https://kanfandelong.github.io/liclock-web-flash/firmware-info.json"
...
void HAL::cheak_firmware_update() {
    ...
    if (LittleFS.exists(CAcert_file)) {
        ...
        ca_cert = new (
            std::nothrow) char[file_size + 1];  // +1为终止符
                                                // 读取证书内容并替换CRLF为LF
        size_t index = 0;
...
run:
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (hal.has_new_firmware) {
            strcpy(hal.update_url[0],
                   doc["updateinfo"]["url"].as<String>().c_str());
            strcpy(hal.update_url[1],
                   doc["updateinfo"]["url1"].as<String>().c_str());
...
#include "images/manual.h"
    String passwd =
        String((esp_random() % 1000000000L) + 10000000L);  // 生成随机密码
            ...
            if (!show_qr) {
                ...
                for (uint8_t y = 0; y < qrcode.size; y++) {
                    ...
                    for (uint8_t x = 0; x < qrcode.size; x++) {
                        display.fillRect(2 * x + 20, 2 * y + 20, 2, 2,
                ...
                sprintf(
                    ...
                    "如果使用的是电脑或手机未跳转至配置界面("
                    "移动数据可能会干扰跳转),"
                    "请扫描二维码打开配置界面或浏览器打开http://192.168.4.1");
...
void HAL::ReqWiFiConfig() {
    ...
    display.drawXBitmap(0, 0, no_wifi_connected_bits, no_wifi_connected_width,
                        no_wifi_connected_height, 0);
    display.display();
    uint32_t last_millis = millis();
    int a = 0;
    while (1) {
...
void refresh_partition_table() {
    ...
    memcpy(partition_size.size_byte, &table[16 * 2 * PARTITION_SPIFFS + 0x8],
           4);
    ...
    if (partition_size.size != size_request) {
        ...
        partition_size.size = size_request;
        memcpy(&table[16 * 2 * PARTITION_SPIFFS + 0x8],
               partition_size.size_byte, 4);
        ...
        Serial.println("\n正在写入");
        if (esp_flash_erase_region(esp_flash_default_chip, 0x8000, 0x1000) !=
            ESP_OK) {
            Serial.println("擦除失败");
            while (1) vTaskDelay(1000);
        }
        if (esp_flash_write(esp_flash_default_chip, table, 0x8000,
                            sizeof(table)) != ESP_OK) {
            Serial.println("写入失败");
            while (1) vTaskDelay(1000);
        }
...
void HAL::wait_input() {
    ...
    if (hal.btn_activelow) {
        ...
        esp_sleep_enable_ext1_wakeup((1LL << PIN_BUTTONR),
                                     ESP_EXT1_WAKEUP_ALL_LOW);
    } else {
        // esp_sleep_enable_ext1_wakeup((1ULL << PIN_BUTTONC) | (1ULL <<
        // PIN_BUTTONL) | (1ULL << PIN_BUTTONR), ESP_EXT1_WAKEUP_ANY_HIGH);
...
static const char esp_rst_str[12][64] = {
    "UNKNOWN_RESRT",   "POWERON_RESET",  "EXT_RESET",      "SW_RESET",
    "PANIC_RESET",     "INT_WDT_RESET",  "TASK_WDT_RESET", "WDT_RESET",
    "DEEPSLEEP_RESET", "BROWNOUT_RESET", "SDIO_RESET"};
bool HAL::init() {
    int16_t total_gnd = 0;
    bool timeerr = false;
    bool initial = true;
...
void HAL::autoConnectWiFi() {
    ...
    if (config[PARAM_SSID] == "") {
        ...
        WiFi.mode(WIFI_STA);
        WiFi.begin(config[PARAM_SSID].as<const char *>(),
                   config[PARAM_PASS].as<const char *>());
    }
    if (!WiFi.isConnected()) {
...
static void set_sleep_set_gpio_interrupt() {
    if (hal.btn_activelow) {
        ...
        esp_sleep_enable_ext1_wakeup((1LL << hal._wakeupIO[1]),
                                     ESP_EXT1_WAKEUP_ALL_LOW);
    } else {
        if (hal.pref.getBool(hal.get_char_sha_key("根据唤醒源翻页")) == true &&
            ...
            esp_sleep_enable_ext1_wakeup((1ULL << PIN_BUTTONC) |
                                             (1ULL << PIN_BUTTONL) |
                                             (1ULL << PIN_BUTTONR),
                                         ESP_EXT1_WAKEUP_ANY_HIGH);
        }
...
void HAL::powerOff(bool displayMessage) {
    if (displayMessage) {
        ...
        // u8g2Fonts.print("已关机");
        display.drawXBitmap(0, 0, poweroff_bits, poweroff_width,
                            poweroff_height, 0);
        display.display();
    }
...
int HAL::getNTPMinute() {
    int res[] = {
        0, 2 * 60, 4 * 60, 6 * 60, 12 * 60, 24 * 60, 36 * 60, 48 * 60,
    };
...
void HAL::checkNightSleep() {
    ...
    // 转换时间数据到分钟
    int sleepStart =
        tmp.substring(0, 2).toInt() * 60 + tmp.substring(3, 5).toInt();
...
bool HAL::copy(File &newFile, File &file) {
    ...
    tray:
        ...
        if (millis() - time >= 2000) {
            ...
            u8g2Fonts.printf("总计：%dKB 剩余：%dKB", fileSize_kb,
                             (fileSize - totalBytesRead) / 1024);
        ...
        if (GUI::waitLongPress(PIN_BUTTONL)) return false;
