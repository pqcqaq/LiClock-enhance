...
inline void pop_buffer() { display.swapBuffer(last_buffer_idx); }
/**
 * @brief  绘制带标题的窗口
 * @param title 标题文本
 * @param x 窗口x坐标（左上角）
 ...
 * @param h 窗口高度
 */
void drawWindowsWithTitle(const char *title, int16_t x, int16_t y, int16_t w,
                          int16_t h) {
    int16_t wchar;
...
uint8_t colorThresholdLimit(uint8_t val1, int8_t val2)  // 颜色阈值限制
{
    ...
    int16_t out = 0;
    // Serial.print("val1_int:" + String(val1_int)); Serial.print(" val2_int:" +
    // String(val2_int)); Serial.println(" tmp:" + String(tmp));
    if (tmp > 255)
        return 255;
    else if (tmp < 0)
        return 0;
...
void msgbox(const char *title, const char *msg) {
    ...
    while (1) {
        if (hal.btnr.isPressing() || hal.btnl.isPressing() ||
            hal.btnc.isPressing())
            break;
        if (millis() - start > 30000) hal.wait_input();
        delay(10);
    }
    pop_buffer();
    hal.unhookButton();
 ...
 */
bool msgbox_yn(const char *title, const char *msg, const char *yes,
               const char *no) {
    // 160*100窗口，圆角5
    constexpr int start_x = (296 - 160) / 2;
    constexpr int start_y = (128 - 96) / 2;
    if (yes == NULL) yes = "确定 (右)";
    if (no == NULL) no = "取消 (左)";
    int16_t w;
    bool result = false;
    hal.hookButton();
 ...
 * @return int类型的选中的菜单项
 */
int menu(const char *title, const menu_item options[], int16_t ico_w,
         int16_t ico_h) {
    ...
    while (options[total].title != NULL) {
        // 统计一共多少，顺便检查是否有图标
        if (options[total].icon != NULL) hasIcon = true;
        ++total;
    }
    ...
    while (1) {
        ...
        if (updated == true) {
            ...
            if (total > number_of_items) {
                ...
                display.fillRoundRect(start_x + 195 + 1, start_y + 15 + barPos,
                                      3, barHeight, 2, 0);
            }
...
void sha256(const char *input, uint8_t output[32], SHA_CTX *ctx) {
    ...
    ets_sha_update(ctx, SHA2_256, (const uint8_t *)input,
                   strlen(input) * 8);      // 更新哈希值
                ...
                if (waitLongPress(PIN_BUTTONC) == true) {
                    ...
                    if (selected == 0) {
                        ...
                        if (options[selected].select == false)
                            ...
                            hal.pref.putBool(
                                sha_option_key[selected],
                                !hal.pref.getBool(sha_option_key[selected],
                                                  false));
                            updated = true;
                        }
                    }
            ...
            for (int i = 0; i < max_items; ++i) {
                ...
                if (options[i + pageStart].select == true) {
                    if (hal.pref.getBool(sha_option_key[i + pageStart])) {
                        display.drawXBitmap(start_x + 5,
                                            item_y + (14 - ico_h) / 2,
                                            select_bits, ico_w, ico_h, 0);
                    } else {
                        display.drawXBitmap(start_x + 5,
                                            item_y + (14 - ico_h) / 2,
                                            no_select_bits, ico_w, ico_h, 0);
                    }
                }
                u8g2Fonts.drawUTF8(
                    start_x + 5 +
                        (options[i + pageStart].select ? ico_w + 2 : 0),
                    item_y + 13, options[i + pageStart].title);
                if (selected == i + pageStart) {
                    display.drawRoundRect(start_x + 3, item_y, 195 - 6, 15, 3,
                                          0);
    ...
    for (int row = 0; row < numRows; row++) {
        for (int col = 0; col < numCols; col++) {
            ...
            if (row == selectedRow && col == selectedCol) {
                display.fillRect(x + 1, y + 1, KEY_WIDTH - 2, KEY_HEIGHT - 2,
                                 GxEPD_BLACK);
 ...
 * @param  with_color 颜色
 */
void drawBMP(FS *fs, const char *filename, bool partial_update, bool overwrite,
             int16_t x, int16_t y, bool with_color) {
    uint8_t input_buffer[3 * input_buffer_pixels];  // 深度不超过24
    uint8_t output_row_mono_buffer[max_row_width /
                                   8];  // 用于至少一行黑白比特的缓冲区
    uint8_t
        output_row_color_buffer[max_row_width / 8];  // 至少一行颜色位的缓冲区
    uint8_t
        mono_palette_buffer[max_palette_pixels / 8];  // 调色板缓冲区深度<=8黑白
    uint8_t color_palette_buffer[max_palette_pixels /
                                 8];  // 调色板缓冲区深度<=8 c/w
    uint16_t rgb_palette_buffer
        [max_palette_pixels];  // 对于缓冲图形，调色板缓冲区的深度<=8，需要7色显示
                ...
                do {
                         ...
                         row++, rowPosition += rowSize)  // 对于每条线
                    {
                        ...
                        for (uint16_t col = 0; col < w;
                             col++)  // 对于每个像素 //width 修补 w
                        {
                            // 是时候读取更多像素数据了？
                            if (in_idx >=
                                in_bytes)  // 好的，24位也完全匹配（大小是3的倍数）
                            {
                                in_bytes =
                                        ...
                                        // (0.114*Blue+0.587*Green+0.299*Red)
                            {
                                case 24:
                                    ...
                                    // whitish = 发白的
                                    // whitish = with_color ? ((red > 0x80) &&
                                    // (green > 0x80) && (blue > 0x80)) : ((red
                                    // + green + blue) > 3 * 0x80); colored =
                                    // (red > 0xF0) || ((green > 0xF0) && (blue
                                    // > 0xF0));                //
                                    // 红色还是黄色？ colored = 有色的 color =
                                    // ((red & 0xF8) << 8) | ((green & 0xFC) <<
                                    // 3) | ((blue & 0xF8) >> 3); // color =
                                    // 颜色 color = 0.114 * float(blue) + 0.587
                                    // * float(green) + 0.299 * float(red);
                                    // //灰度转换
                                    color = (114 * blue + 587 * green +
                                             299 * red + 500) /
                                            1000;  // 灰度转换
                                    break;
                                case 16: {
                            ...
                            // Serial.print("x + col:" + String(x + col));
                            // Serial.println(" yrow:" + String(yrow));
                            if (depth == 1)  // 位深为1位，直接绘制
                            {
                                if (whitish) color = GxEPD_WHITE;
                                // else if (colored && with_color) color =
                                // GxEPD_COLORED;
                                else
                                    color = GxEPD_BLACK;
                                display.drawPixel(x + col, yrow,
                                                  color);  // 原始
                            } else  // 位深为24，16，8位 使用像素抖动绘制
                            {
                                ...
                                /*float B = 0.1;
                                  ...
                                  color = (color - 127.5 * (1 - B)) * K + 127.5
                                  * (1 + B); if (color > 255) color = 255; else
                                  if (color < 0) color = 0;*/
                                // 分段抖动，每3行抖动一次
                                bmp8[x + col][yrow1] = color;
                                if (x + col ==
                                    (w - 1))  // X轴填满，换行 //width 修补 w
                                {
                                    yrow1++;  // Y轴进位
                                    // 首次需要存入6行数据再抖动
                                    // ，中间每次在012后面存入3行
                                    if (yrow1 == 6 ||
                                        ...
                                        // Serial.print("y_max0：");
                                        // Serial.println(y_max0);
                                        yrow1 = 2;  // Y轴进位回到第3行，012

                                        for (uint16_t y = 0; y <= y_max0;
                                             y++)  // height width
                                        {
                                            for (uint16_t x = 0; x < w;
                                                 x++)  // width 修补 w
                                            {
                                                if (ddxhFirst == 1 ||
                                                    y != 0)  // 第一次对01234行抖动处理
                                        ...
                                        // 绘制像素点 bmp[x][y] x轴绘制需全部完
                                        // y轴只绘制前5行 bmp图片Y轴绘制初始位置
                                        if (flip == 1 && yrow != 0)
                                            yrow_old = yrow + 5;
                                        else if (flip == 0 &&
                                                 yrow != (height - 1))
                                        ...
                                        for (uint16_t y = 0; y < y_max1; y++) {
                                            ...
                                            {
                                                /*Serial.print("x:" +
                                                  String(x)); Serial.print(" y:"
                                                  + String(y)); Serial.println("
                                                  bmp8:" +
                                                  String(bmp8[x][y]));*/
                                                /*if (yrow_old > 110) {
                                                  Serial.print("yrow_old:");
                                                  Serial.println(yrow_old);
                                                  }*/
                                                display.drawPixel(x, yrow_old,
                                                                  bmp8[x][y]);
                                            }
                                            // Y轴进位
...
uint8_t (*bmp8)
    [16 +
     1];  // 创建像素抖动缓存二维数组（先不定长度），JPG最大的输出区块16+1行缓存（缓存上一次的最后一行）
...
void drawJPG(String name, FS fs) {
    ...
    // if (jpgWidth != jpgHeight) display.setRotation(jpgWidth > jpgHeight ? 3 :
    // 0);

    // 设置缩放 1-2-4-8
    uint16_t scale = 1;
    for (scale = 1; scale <= 8; scale <<= 1) {
        if (jpgWidth <= display.width() * scale &&
            jpgHeight <= display.height() * scale) {
            if (scale > 1) {
                scale = scale >>
                        1;  // 屏幕太小，缩得比屏幕小就看不清了，回到上一个缩放
            }
            break;
        }
    }
    ...
    do {
        uint8_t error;
        error =
            TJpgDec.drawFsJpg(x_center, y_center, name, fs);  // 发送文件和坐标
        String str = "";
        if (error == 1)
            str = "被输出功能中断";  // Interrupted by output function
        else if (error == 2)
            str = "设备错误或输入流的错误终止";  // Device error or wrong
                                                 // termination of input stream
        else if (error == 3)
            ...
            str = "数据格式错误（可能是损坏的数据）";  // Data format error (may
                                                       // be broken data)
        else if (error == 7)
    ...
    if (blockCount_x >= jpgWidth) {
        ...
        for (uint16_t y = 0; y <= y_max; y++)  // height width
        {
            for (uint16_t x = 0; x < jpgWidth; x++) {
                ...
                if (x != jpgWidth - 1)
                    bmp8[x + 1][y + 0] = colorThresholdLimit_jpg(
                        bmp8[x + 1][y + 0], (err * 7) / 16);
                if (x != 0)
                    bmp8[x - 1][y + 1] = colorThresholdLimit_jpg(
                        bmp8[x - 1][y + 1], (err * 3) / 16);
                if (1)
                    bmp8[x + 0][y + 1] = colorThresholdLimit_jpg(
                        bmp8[x + 0][y + 1], (err * 5) / 16);
                if (x != jpgWidth - 1)
                    bmp8[x + 1][y + 1] = colorThresholdLimit_jpg(
                        bmp8[x + 1][y + 1], (err * 1) / 16);
            }
        }  // 像素抖动结束
        ...
        if (y == jpgHeight + y_start) {
            y_p_max +=
                (jpgHeight + y_start - 1) - (y_p + y_p_max);  // 127-（119+7）
        }
        ...
        {
            for (uint16_t x = 0; x < jpgWidth; x++) bmp8[x][0] = bmp8[x][h - 1];
        } else  // 第一次之后
        {
            for (uint16_t x = 0; x < jpgWidth; x++) bmp8[x][0] = bmp8[x][h];
        }
