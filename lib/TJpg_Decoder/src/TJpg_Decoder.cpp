...
JRESULT TJpg_Decoder::drawFsJpg(int32_t x, int32_t y, fs::File inFile) 
{
  ...
  // 提取图像并渲染
  if (jresult == JDR_OK) {
	//Serial.println(F("解压缩图像"));
    jresult = jd_decomp(&jdec, jd_output, jpgScale);
	/*if(jresult!=JDR_OK) 
	{
		Serial.print(F("解压缩图像失败，错误代码"));
		Serial.println(jresult);
	}*/
  }
  /*else 
  {
	  Serial.print(F("提取图像失败，错误代码"));
	  Serial.println(jresult);
  }*/
...
/***************************************************************************************
** Function name:           getFsJpgBlock
** Description:             获取jpg图片的解码块大小，FS系统	
***************************************************************************************/
// 获取块大小
JRESULT TJpg_Decoder::getFsJpgBlock(uint8_t *msx, uint8_t *msy, const char *pFilename, fs::FS &fs) 
{
  // Check if file exists
  if ( !fs.exists(pFilename) )
  {
    Serial.println(F("找不到Jpeg文件"));
    return JDR_INP;
  }

  return getFsJpgBlock(msx, msy, fs.open( pFilename, "r"));
}
// 获取块大小
JRESULT TJpg_Decoder::getFsJpgBlock(uint8_t *msx, uint8_t *msy, fs::File inFile)
{
  JDEC jdec;
  JRESULT jresult = JDR_OK;
