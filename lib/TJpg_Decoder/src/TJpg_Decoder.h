...
#ifdef TJPGD_LOAD_FFS
  JRESULT drawFsJpg (int32_t x, int32_t y, const char *pFilename, fs::FS &fs = LittleFS);
  JRESULT drawFsJpg (int32_t x, int32_t y, const String& pFilename, fs::FS &fs = LittleFS);
  JRESULT drawFsJpg (int32_t x, int32_t y, fs::File inFile);

  JRESULT getFsJpgSize(uint16_t *w, uint16_t *h, const char *pFilename, fs::FS &fs = LittleFS);
  JRESULT getFsJpgSize(uint16_t *w, uint16_t *h, const String& pFilename, fs::FS &fs = LittleFS);
  JRESULT getFsJpgSize(uint16_t *w, uint16_t *h, fs::File inFile);
  
  JRESULT getFsJpgBlock(uint8_t *msx, uint8_t *msy, const char *pFilename, fs::FS &fs = LittleFS);
  JRESULT getFsJpgBlock(uint8_t *msx, uint8_t *msy, fs::File inFile);
